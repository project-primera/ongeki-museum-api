...
{
    private readonly List<string> _musicJsonUrls = [
        configuration[$"Ongeki:MusicJsonUrl"] ?? throw new InvalidOperationException("Ongeki:MusicJsonUrl is not set"),
    ];
    ...
    public async Task FetchAndSaveOfficialMusicAsync()
    {
        ...
        foreach (var musicJsonUrl in _musicJsonUrls)
        {
            try
            {
